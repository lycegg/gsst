...
long whole_count=0;
int file_stat_size;
#define reading_loop 60
unsigned char*cuda_host_read[reading_loop],*cuda_host_write[120];
//int cuda_hr_idx;
void myread(){
    ...
    unsigned char*buf=cuda_host_read[readed.size()%reading_loop];
...
void mymalloc(){
    int sum=0;
    for(int i=0;i<tomalloc.size();i++){
        tomalloc[i].second=(tomalloc[i].second/16)*16;
        sum+=tomalloc[i].second;
    }
    unsigned char*buf;
    cudaMallocHost(&buf,sum);
    printf("trying to malloc %lld\n",sum);
    sum=0;
    for(auto ato:tomalloc){
        *((unsigned char**)ato.first)=buf+sum;
        sum+=ato.second;
    }
}
int main(int argc, char **argv){
    ...
    int threads_perSM=std::stoi(argv[5]);//cudaHostRegister
    fin=fopen(argv[1],"rb");fout=fopen(argv[2],"wb");
    unsigned char**reading_buf_cpu,**whole_reading_buf_gpu[2],**whole_writing_buf_gpu[2];//,*writing_buf_cpu[parallelism]
    tomalloc.push_back(ppi((unsigned char*)&reading_buf_cpu,parallelism*sizeof(void**)));
    //cudaMallocHost();
    tomalloc.push_back(ppi((unsigned char*)&whole_reading_buf_gpu[0],parallelism*sizeof(void*)));
    tomalloc.push_back(ppi((unsigned char*)&whole_reading_buf_gpu[1],parallelism*sizeof(void*)));
    tomalloc.push_back(ppi((unsigned char*)&whole_writing_buf_gpu[0],parallelism*sizeof(void*)));
    tomalloc.push_back(ppi((unsigned char*)&whole_writing_buf_gpu[1],parallelism*sizeof(void*)));
    unsigned char **whole_preading_buf_gpu[2],**whole_pwriting_buf_gpu[2];
    unsigned char** whole_codelen_gpu[2];
    tomalloc.push_back(ppi((unsigned char*)&whole_codelen_gpu[0],parallelism*sizeof(void*)));
    tomalloc.push_back(ppi((unsigned char*)&whole_codelen_gpu[1],parallelism*sizeof(void*)));
    unsigned char** whole_pcodelen_gpu[2];
    unsigned long long** whole_code_gpu[2];
    tomalloc.push_back(ppi((unsigned char*)&whole_code_gpu[0],parallelism*sizeof(void*)));
    tomalloc.push_back(ppi((unsigned char*)&whole_code_gpu[1],parallelism*sizeof(void*)));
    ...
    for(int i=0;i<120;i++){
        tomalloc.push_back(ppi((unsigned char*)(cuda_host_write+i),4*1024*1025));    
    }
    for(int i=0;i<reading_loop;i++)
        tomalloc.push_back(ppi((unsigned char*)(cuda_host_read+i),65536*256+65536));    
    //CHECK(cudaGetLastError());
    //CHECK(cudaDeviceSynchronize());
        tomalloc.push_back(ppi((unsigned char*)&codelen,parallelism*256));
        tomalloc.push_back(ppi((unsigned char*)&code,parallelism*256*8));
        tomalloc.push_back(ppi((unsigned char*)&RawPerThread,parallelism*4));
        tomalloc.push_back(ppi((unsigned char*)&numthreads,parallelism*4));
    //CHECK(cudaGetLastError());
    //CHECK(cudaDeviceSynchronize());
    auto mlc_start=clock();
    mymalloc();
    CHECK(cudaGetLastError());
    CHECK(cudaDeviceSynchronize());
    printf("malloc_time=%lld\n",clock()-mlc_start);
    ...
    for(int process_idx=0;process_idx<readed.size();process_idx+=parallelism){
        ...
        for(int iter=0;iter<parallelism;iter++){
            ...
            if(process_idx>=3*parallelism){
                //cudaFreeHost(readed[writeidx].first);
                fwrite(to_write[writeidx].first,1,to_write[writeidx].second,fout);
                //cudaFreeHost(to_write[writeidx].first);
                writeidx++;
            }
        }
        iotime+=clock()-time1;
        ...
        for(int iii=0;iii<last_cnt;iii++){
            unsigned char* tmp_buf=cuda_host_write[to_write.size()%120];
            //cudaMallocHost(&tmp_buf,4*1024*1025);
    ...
    for(int iiii=writeidx;iiii<to_write.size();iiii++){
        ...
        //cudaFreeHost(to_write[iiii].first);
    }
    iotime+=clock()-time1;
    sum_time=clock()-time0;
