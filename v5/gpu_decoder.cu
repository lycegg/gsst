...
int main(int argc, char **argv){
    ...
    int file_stat_size=file_stat.st_size;
    ...
    int streamcnt=0;
    long time0=clock();
    while(whole_count<file_stat_size){
        ...
        for(int iii=0;iii<parallelism;iii++){
            reading_buf_cpu[iii]=whole_reading_file+whole_count;
            //writing_buf_cpu[iii]=(unsigned char*)malloc(4*1024*1025);
            //long current_pos = 
            //if(debug_)
            //printf("cur_pos:%lld\n",ftell(fin));
            if(whole_count>=file_stat_size)break;
            cnt++;
