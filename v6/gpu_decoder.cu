...
const int MAX_READ_AHEAD = 211;
const int MAX_PROCESS_AHEAD = 211;

std::queue<ppi> readQueue;
std::queue<ppi> writeQueue;

std::mutex readMtx, writeMtx;//, procAheadMtx
std::condition_variable readCV, writeCV;//, procAheadCV

//int processingAhead = 0;
bool doneReading = false,doneprocessing=false;
//int totalUnits = 0; // 文件总单元数，读线程设置

//unsigned char**reader_buf,**writer_buf;
...
void reader() {
    long readed=0;
    while (readed<file_stat_size) {
        int comped_size;
        fread(&comped_size,4,1,fin);
        unsigned char*newbuf=(unsigned char*)malloc(comped_size);
        fread(newbuf,1,comped_size,fin);
        // 读取逻辑
        //part = totalUnits++; // 模拟读取
        std::unique_lock<std::mutex> l(readMtx);
        readCV.wait(l, []{ return readQueue.size() < MAX_READ_AHEAD; });
        readQueue.push(ppi(newbuf,comped_size));
        readed+=4+comped_size;
        if(readed>=file_stat_size)doneReading = true;
        l.unlock();
        readCV.notify_all();
    }
    //doneReading = true;
...
void processor() {
    ...
    while (true) {
        ...
        { // 获取待处理单元
            std::unique_lock<std::mutex> l(readMtx);
            cur_doneReading=doneReading;
            readCV.wait(l, []{ return readQueue.size()>=parallelism || doneReading; });
            if (readQueue.empty() && doneReading) break;
            //part = readQueue.front(); readQueue.pop();
            for(int i=0;i<parallelism;i++){
                if(readQueue.empty())break;
                cur_round.push(readQueue.front());
                readQueue.pop();
            }
            readCV.notify_all();
        }

        { // 控制处理不快于写入MAX_PROCESS_AHEAD个
            //std::unique_lock<std::mutex> l(procAheadMtx);
            std::unique_lock<std::mutex> l(writeMtx);
            //procAheadCV.wait(l, []{ return writeQueue.size() < MAX_PROCESS_AHEAD; });
            writeCV.wait(l, []{ return writeQueue.size() < MAX_PROCESS_AHEAD; });
            //++processingAhead;
        }
        ...
        for(int iii=0;iii<parallelism;iii++){
            if(cur_round.empty())break;
            auto tmp_que=cur_round.front(); cur_round.pop();
            reading_buf_cpu[iii]=tmp_que.first;
            auto mx=tmp_que.second;
            //printf("size=%d\n",mx);
            //fflush(stdout);
            ...
            for(int kk=0;kk<8;kk++) {
                int k=(kk+1)%8;
                //printf("codecount_cur=%lld\n",(long)codecount);fflush(stdout);
                for(int j=0;j<codecount[k];j++){
                    codelen[i]=k+1;
                    //printf("codecount=%lld\n",(long)codecount);fflush(stdout);
                    //printf("pos=%d\n",pos);
                    //fflush(stdout);
            ...
            //long time1=clock();
            cudaMemcpyAsync(reading_buf_gpu[iii],reading_buf_cpu[iii]+gpu_start_pos,1+4*numthreads[iii]-1+textsize,cudaMemcpyHostToDevice,cuda_streams[cur_s]);
            //CHECK(cudaGetLastError());
            //CHECK(cudaDeviceSynchronize());
            cudaMemcpyAsync(code_gpu[iii],code,256*8,cudaMemcpyHostToDevice,cuda_streams[cur_s]);
            cudaMemcpyAsync(codelen_gpu[iii],codelen,256,cudaMemcpyHostToDevice,cuda_streams[cur_s]);
        ...
        //long time1=clock();
        cudaMemcpyAsync(p_RawPerThreadGpu,RawPerThread,parallelism*sizeof(int),cudaMemcpyHostToDevice,cuda_streams[cur_s]);
        //for(int iii=0;iii<cnt;iii++){
        cudaMemcpyAsync(preading_buf_gpu,reading_buf_gpu, parallelism*sizeof(int*),cudaMemcpyHostToDevice,cuda_streams[cur_s]);
        cudaMemcpyAsync(pwriting_buf_gpu,writing_buf_gpu, parallelism*sizeof(int*),cudaMemcpyHostToDevice,cuda_streams[cur_s]);
        cudaMemcpyAsync(pcodelen_gpu,codelen_gpu, parallelism*sizeof(int*),cudaMemcpyHostToDevice,cuda_streams[cur_s]);
        cudaMemcpyAsync(pcode_gpu,code_gpu, parallelism*sizeof(int*),cudaMemcpyHostToDevice,cuda_streams[cur_s]);
        //long time0=clock();
        decode<<<cnt,threads_perSM,0,cuda_streams[cur_s]>>>(preading_buf_gpu,pwriting_buf_gpu,pcode_gpu,pcodelen_gpu,p_RawPerThreadGpu);
        //if(debug_)
        //    printf("finish_time=%lld\n",clock());
        //sum_time+=clock()-time0;
        for(int ijk=0;ijk<cnt;ijk++)free(reading_buf_cpu[ijk]);
        if(streamcnt){
            std::lock_guard<std::mutex> l(writeMtx);
            for(int iii=0;iii<last_cnt;iii++){
                auto tmp_buf=(unsigned char*)malloc(4*1024*1025);
                assert(rawsize[1-cur_s][iii]<4*1024*1025);
                cudaMemcpyAsync(tmp_buf,whole_writing_buf_gpu[1-cur_s][iii],rawsize[1-cur_s][iii],cudaMemcpyDeviceToHost,cuda_streams[1-cur_s]);
                writeQueue.push(ppi(tmp_buf,rawsize[1-cur_s][iii]));
            }
            writeCV.notify_all();
        }
        last_cnt=cnt;
        ...
        if(readQueue.empty()&& cur_doneReading){ // 放入写入队列
            //printf("ending\n");
            //fflush(stdout);
            std::lock_guard<std::mutex> l(writeMtx);
            ...
            for(int iii=0;iii<last_cnt;iii++){
                auto tmp_buf=(unsigned char*)malloc(4*1024*1025);
                assert(rawsize[1-cur_s2][iii]<4*1024*1025);
                cudaMemcpyAsync(tmp_buf,whole_writing_buf_gpu[1-cur_s2][iii],rawsize[1-cur_s2][iii],cudaMemcpyDeviceToHost,cuda_streams[1-cur_s2]);
                writeQueue.push(ppi(tmp_buf,rawsize[1-cur_s2][iii]));
            }
            doneprocessing=true;
            writeCV.notify_all();
            return;
...
void writer() {
    ...
    while (true) {
        ...
        { // 取写入单元
            std::unique_lock<std::mutex> l(writeMtx);
            writeCV.wait(l, []{ return writeQueue.size()>=3*parallelism||doneprocessing; });
            item = writeQueue.front(); writeQueue.pop();
            //++finished;
        }
        // 写入逻辑
        //auto item = writeQueue.front(); writeQueue.pop();
        
        fwrite(item.first,1,item.second,fout);
        free(item.first);
        { // 处理信号释放
            //std::lock_guard<std::mutex> l(procAheadMtx);
            std::unique_lock<std::mutex> l(writeMtx);
            //procAheadCV.notify_all();
            writeCV.notify_all();
            if(doneprocessing&&writeQueue.empty())return;
            //--processingAhead;
        }
    }
}
...
int main(int argc, char **argv){
    ...
    debug_=std::stoi(argv[3]);
    parallelism=std::stoi(argv[4]);
    threads_perSM=std::stoi(argv[5]);
    ...
    auto time0=clock();
    std::thread r(reader);
    std::thread p(processor);
    std::thread w(writer);
    r.join();
    p.join();
    w.join();
    auto sum_time=clock()-time0;
    //for(auto ato:to_write){
    //    fwrite(ato.first,1,ato.second,fout);
    //    free(ato.first);
    //}
    printf("sum_time=%lld\n",sum_time);
