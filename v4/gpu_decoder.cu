...
int main(int argc, char **argv){
    ...
    unsigned char **preading_buf_gpu,**pwriting_buf_gpu;
    unsigned char* codelen_gpu[parallelism];
    unsigned char** pcodelen_gpu;
    unsigned long long* code_gpu[parallelism];
    unsigned long long** pcode_gpu;
    int*p_RawPerThreadGpu;
    for(int i=0;i<parallelism;i++){
        ...
        cudaMalloc((void **)&p_RawPerThreadGpu, parallelism*sizeof(int));
        cudaMalloc((void **)&preading_buf_gpu, parallelism*sizeof(int*));
        cudaMalloc((void **)&pwriting_buf_gpu, parallelism*sizeof(int*));
        cudaMalloc((void **)&pcodelen_gpu, parallelism*sizeof(int*));
        cudaMalloc((void **)&pcode_gpu, parallelism*sizeof(int*));
    long sum_time=0;
    long sum_time_mem=0;
    while(!is_end_of_file(fin)){
        ...
        for(int iii=0;iii<parallelism;iii++){
            ...
            int pos=4+17;
            rawsize[iii]=DESERIALIZE(reading_buf_cpu[iii])*256+reading_buf_cpu[iii][3];
            if(debug_)
            printf("rawsize=%d\n",rawsize[iii]);
        ...
        CHECK(cudaDeviceSynchronize());
        long time1=clock();
        cudaMemcpy(p_RawPerThreadGpu,RawPerThread,parallelism*sizeof(int),cudaMemcpyHostToDevice);
        //for(int iii=0;iii<cnt;iii++){
        cudaMemcpy(preading_buf_gpu,reading_buf_gpu, parallelism*sizeof(int*),cudaMemcpyHostToDevice);
        cudaMemcpy(pwriting_buf_gpu,writing_buf_gpu, parallelism*sizeof(int*),cudaMemcpyHostToDevice);
        cudaMemcpy(pcodelen_gpu,codelen_gpu, parallelism*sizeof(int*),cudaMemcpyHostToDevice);
        cudaMemcpy(pcode_gpu,code_gpu, parallelism*sizeof(int*),cudaMemcpyHostToDevice);
        CHECK(cudaGetLastError());
        CHECK(cudaDeviceSynchronize());
        long time0=clock();
